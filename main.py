from browser import document, html, timer, window, template, local_storage, console
...
def card_name (card) :
	num = CardImg.TRANSLATE_NUM [ card.getNum () ]
	suit = CardImg.TRANSLATE_SUIT [ card.getSuit () ]
	return f'{ num }_of_{ suit }.svg'
...
def switch_card_sources (evt = None) :
	'switch to the next set of card source images'

	old = CARD_SOURCES [pos]

	global pos
	pos = (pos + 1) % len (CARD_SOURCES)
	card_dir = CARD_SOURCES [pos]

	new = card_dir
	console.log (f'switching deck from { old } to { new }')

	global app

	# use copyOfDeck because deck is empty and getting cards from board
	# doesnt get aces

	#cards = app._board.getAllCards ()
	#cards = app._deck.getCards ()
	cards = app._copyOfDeck.getCards ()
	debug (f'cards = { cards }')

	for card in cards:

		# erase old card image from board
		image = app._card2ImgDict [ id (card) ]
		image.erase ()

		# load new card image
		image_name = card_name (card)
		card_url = card_dir + image_name

		#cardimg = CardImg (card, app._canv)
		cardimg = image
		fabric.Image.fromURL (card_url , cardimg._onload)

		# update mapping from card to image

		app._card2ImgDict [ id (card) ] = cardimg

		debug (f'card = { card } : { image_name }')
		debug (f'new url = { card_url }')

	# update mapping and redraw board

	app._boardGui._card2ImgDict = app._card2ImgDict
	app._boardGui.displayLayout ()
	app.highlightMovableCards ()
	app.markGoodCards ()
...
class CardImg:
	...
	def __init__(self, card: Card, canv):
		...
		card_dir = CARD_SOURCES [0]
		fabric.Image.fromURL(
			card_dir + self._image_name,
			self._onload
		)
